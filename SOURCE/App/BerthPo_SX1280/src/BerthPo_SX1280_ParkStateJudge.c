...
uint8_t BerthPo_SendAlarmPackage(uint8_t frameType)
{
    ...
    for(i = 0; i < 6; i++)
    {
        m_sendPackage.packageHeader.deviceNumber[i] =
            controlConfig.nodeConfig.idNumber[i];
        m_sendPackage.packageHeader.keyNumber[i] =
            controlConfig.nodeConfig.keyNumber[i];
    }
    ...
    m_sendPackage.txPayLoad[2] =
        controlConfig.paramConfig.alarmStatus;      //车位状态
    ...
    switch (m_sendPackage.txPayLoad[2] >> 6)
    {
        ...
        case 3:
        {
            if (tagConfigSymple.outBottomFlag == 0)
                m_sendPackage.txPayLoad[8] = ((uint8_t)(
                                                  controlConfig.paramConfig.EMDataBottom_z));//z固定本底值
            else
                m_sendPackage.txPayLoad[8] = ((uint8_t)(
                                                  sensorRm3100.rm3100DynamicBoottom.EMDataBottom_z));//z动态本底值
            break;
        }
    }
    m_sendPackage.packageHeader.frameLens = 9 + 2;
    uint8_t frameTotalLens = 18 + m_sendPackage.packageHeader.frameLens;
    uint8_t m_sendBuf[9 + 2 + 18] = {0};
    ...
    while(m_trySendTimes--)
    {
        ...
        if(SX1280_Operation.SX1280_GetTxDoneFlag() == 0x5A)
        {
            ...
            for(uint8_t i = 0; i < 2; i++)   //数据发送成功LED1快速闪烁三次
            {
